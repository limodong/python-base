...
for item in students:
    if item.get("sex") == "女":
        ...
        female_contacts.append({"name": item.get("name"), "tel": item.get("tel")})
    if item.get("age") < 25:
        young_females.append(item)
    if "陈" in item.get("name"):
        chen_students.append(item)
    if item.get("tel")[-1] == "1":
        tel_end_with_1.append(item)
    all_names.append(item.get("name"))
    total_age += item.get("age")
    result.get("names").append(item.get("name"))
    result.get("ages").append(item.get("age"))
    if item.get("id") == 796997:
        find = {"name": item.get("name"), "tel": item.get("tel")}
    if item.get("sex") == "男" and item.get("age") > 28:
        print("True", f"姓名：{item.get('name')}", sep="-")
    else:
        print("False")
    if item.get("sex") == "女" and item.get("age") < 28:
        ...
        print(False)
# print(f"所有学生的年龄总和：{total_age}")
# print(f"所有学生的平均年龄：{total_age/len(students)}")
# print(result)
print(
    "未找到" if find == None else f"姓名：{find.get('name')}，电话：{find.get('tel')}"
)
